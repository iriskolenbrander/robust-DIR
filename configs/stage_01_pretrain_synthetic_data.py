...
from configs.config_paths import ROOT_DATA_SYNTHETIC, ROOT_OUTPUT, ROOT_CHECKPOINTS
from datasets.dataset_generated_shapes_random import load_datasets
...
def parse_arguments():
    ...
    parser.add_argument("-rw", '--reg_weight', type=float, metavar='', default=0.75,  help='Regularization (smoothing) weight')
    ...
    parser.add_argument('-wandb', '--mode_wandb', type=str, metavar='', default='online', help='online", "offline" or "disabled')
    config = parser.parse_args()

    config.root_output = ROOT_OUTPUT
    config.root_checkpoints = ROOT_CHECKPOINTS
    config.root_data = ROOT_DATA_SYNTHETIC

    config.imgsize = (192, 192, 192)
    config.run_name_pretrained = "None"
...
if __name__ == '__main__':
    # Initialize the root directories and configuration
    now = datetime.now()
    ...
    """ DATASET """
    train_dataset, val_dataset = load_datasets(shape=config.imgsize,
                                 root_data=config.root_data)

    config.metrics_dict = dict(dice=True, max_label=26)
    config.voxel_spacing = val_dataset.voxel_spacing
