...
class SpatialTransformer(nn.Module):
    ...
    def warp_landmarks(self, src, flow):
        # move channels dim to last position
        # also not sure why, but the channels need to be reversed
        # flow_ = flow.permute(0, 2, 3, 4, 1)
        # idx = torch.round(src).to(torch.int64).squeeze()
        # displacements = flow_[:, idx[:, 0].tolist(), idx[:, 1].tolist(), idx[:, 2].tolist(), :].squeeze()
        # if self.unity:
        #     displacements = displacements * torch.Tensor(data=flow_.shape[1:4]).to(flow_.device)
        # points_warped_old = src + displacements
        # NEW
        src_temp = torch.div(src.to(flow.device), torch.tensor(flow.shape[2::]).to(flow.device)) * 2 - 1
        ...
        points_warped = src.to(flow.device) + pred_xyz.unsqueeze(0)
